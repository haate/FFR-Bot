...
class Races(commands.Cog):
    ...
    async def startrace(self, ctx, *, name=None):
        ...
        race.role = await ctx.guild.create_role(
            name=race.id, reason="role for a race"
        )
    ...
    async def join(self, ctx, id=None, name=None):
        ...
        teamslist[id][ctx.author.id] = dict(
            [
                ("name", name),
                ("members", [[ctx.author.display_name, ctx.author.id]]),
            ]
        )
    ...
    async def spectate(self, ctx, id):
        ...
        if id:
            await race.channel.send(
                "%s is now cheering you on from the"
    ...
    async def removeraceroom(self, ctx, time=0):
        ...
        await channel.delete(
            reason="bot deleted channel because the race ended"
        )
    ...
    async def ff1seed(self, ctx):
        await ctx.channel.send(
            "{0:-0{1}x}".format(random.randint(0, 4294967295), 8)
        )
    ...
    async def multi(self, ctx, raceid: str = None):
        ...
        try:
            ...
            link = await self.multistream(
                race, all=True, discord=True, ctx=ctx
            )
            await ctx.channel.send(link)
            ...
            if link is None:
                await ctx.channel.send(
                    "There is no race with" + " that 5 character id"
                )
            else:
                await ctx.channel.send(link)
    ...
    async def stream(self, ctx):
        for player in ctx.message.mentions:
            try:
                await ctx.channel.send(
                    r"https://www.twitch.tv/{}".format(
                        self.twitchids[str(player.id)]
                    )
                )
            except KeyError:
                await ctx.channel.send(

...
from ..common.mongo_classes import SelfAssignableRole
...
class Roles(commands.Cog):
    ...
    async def add_self_assignable_role(
        ...
        guild = ctx.guild
        new_roles: Set[SelfAssignableRole]
        ...
        for i in range(len(args) // 2):
            ...
            new_roles.add({"role_id": role.id, "description": args[2 * i + 1]})
        ...
        async def add_roles() -> None:
            ...
            config.role.add_self_assignable_roles(guild.id, new_roles)
    ...
    async def remove_self_assignable_role(
        ...
        for role_name in args:
            try:
                ...
                await ctx.message.delete()
                raise Exception("couldn't find role name: " + role_name)
            stale_role_ids.add(role.id)

            question += role.name + "\n"

        async def remove_roles() -> None:

            config.role.remove_self_assignable_roles(guild.id, stale_role_ids)
